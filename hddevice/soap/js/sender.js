...
WEB_SOCKET_SWF_LOCATION = "http://"+document.domain+":3333/swf/WebSocketMain.swf";
...
window.onload = function()
{
	...
	ws.onopen = function() {
		// 发送数据
		//var doctor = document.getElementById("doctor_code").value();
		ws.send(JSON.stringify({"type":"connect","name":"xx"}));
	}
	  ...
	  switch(data['type']){
	        ...
	        case 'send':
				...
				if(typeof(show_msg)=="function"){
	        		...
	        		alert('from_client_ids:'+data['from_client_id'] + ' to_client_id:' + data['to_client_id'] + '消息:' +data['content'] + '时间:' + data['time']);
	        	}
	      	  break;
	  }
	};
