...
function times(){
   		//当前时间段
		$a1_begin = strtotime(date("Y-m-d")." 07:00:00");
		$a1_end = strtotime(date("Y-m-d")." 12:30:00");
		
		$a2_begin = strtotime(date("Y-m-d")." 12:30:00");
		$a2_end = strtotime(date("Y-m-d")." 20:00:00");
		
		$a3_begin = strtotime(date("Y-m-d")." 20:00:00");
		$a3_end = strtotime(date("Y-m-d")." 23:00:00");
		
		$noon_flag = "";
		$now = time();
		if($now>$a1_begin&&$now<$a1_end){
			$noon_flag = "上午";
		}
		if($now>$a2_begin&&$now<$a2_end){
			$noon_flag = "下午";
		}
		if($now>$a3_begin&&$now<$a3_end){
			$noon_flag = "晚上";
		}
		return $noon_flag;
...
function getOpVal($op_name){
	return M("tbl_op")->where("op_name='".$op_name."'")->getField("op_val");
}
function getDeptNameByCode($dept_code){
	return M("dept_info")->where("dept_code='".$dept_code."'")->getField("dept_name");
}
function getShowRoomName($room_id){
	return M("tbl_room_list")->where("room_id=".$room_id)->getField("real_id");
}
function getDoctorNameByCode($doctor_code){
	return M("doctor_info")->where("doctor_code='".$doctor_code."'")->getField("doctor_name");
}
function getDoctorCodeByName($doctor_name){
	return M("doctor_info")->where("doctor_name='".$doctor_name."'")->getField("doctor_code");
}
function getDocIsOffLine($doctor_code,$dept_code,$expert){
	return M("client_uid")->where("uid='".$doctor_code."',dept_code='".$dept_code."',expert=".$expert)->getField("is_over");
}
//生成固定长度随机数
