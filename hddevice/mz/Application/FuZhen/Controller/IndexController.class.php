...
public function insertPat($pat_code,$where2){
	...
	for($i=0;$i<count($row);$i++){
		...
		$data['noon_flag']=$row[$i]['noon_flag'];
		...
		**/
		...
		if($row[$i]['reg_seq_no']<0){
			$weight = str_replace("-","",$row[$i]['reg_seq_no']);
			$data['weight']	= 1000+$weight;
		}else{
		...
		if($row[0]['pat_code']=='120214101000'){
			$row[0]['is_jz'] = 0;
		}else{
		...
		if(!$local_flag){
			if(trim($row[$i]['doctor_code'])!=""){
				...
				switch($row[$i]['reg_type']){
					case "专家":
					$data['room']= M("client_uid")->where("uid='".trim($row[$i]['doctor_code'])."' and expert=1 and dept_code='".$row[$i]['dept_code']."'")->getField("room_id");
			
					break;
					case "普通":
					$data['room']= M("client_uid")->where("uid='".trim($row[$i]['doctor_code'])."' and expert=0 and dept_code='".$row[$i]['dept_code']."'")->getField("room_id");
					break;
					default:
					$data['room'] = 0;
					break;
	...
	if(count($row)==0){
		...
		if(count($patlist)>0){
			...
			**/
			...
			$cda['reg_seq_no'] = $patlist[0]['reg_seq_no'];
...
public function baodao($str='',$num=0,$zhenqu=''){
	...
	for($x=0;$x<count($a_ary);$x++){
		...
		$my_dept_name = getDeptNameByCode($hd[0]['dept_code']);
		/**
		**患者报到插入记录数据开始*************************************************
		**/
		$his_code = M("tbl_op")->where("op_name='his_code'")->getField("op_val");
		$cda['pat_name'] = $hd[0]['pat_name'];
		$cda['dept_name'] = $hd[0]['dept_name'];
		$cda['reg_type'] = $hd[0]['reg_type'];
		$cda['reg_grade'] = $hd[0]['reg_grade'];
		$cda['doctor_name'] = $hd[0]['doctor_name'];
		$cda['reg_id'] = $hd[0]['reg_id'];
		$cda['pat_code'] = $hd[0]['pat_code'];
		$cda['dept_code'] = $hd[0]['dept_code'];
		$cda['doctor_code'] = $hd[0]['doctor_code'];
		$cda['reg_seq_no'] = $hd[0]['reg_seq_no'];
		$cda['op_name'] = "自助报到";
		$cda['op_from'] = $zhenqu;
		$cda['cdate'] = date("Y-m-d H:i:s");
		...
		**/
		if($hd[0]['withdraw_flag']==0){
			$rel['success'] = 1;
			$w_num = M("register_assign")->where("dept_code='".$hd[0]['dept_code']."' and reg_type='".$hd[0]['reg_type']."' and ((is_jz=1 and status=5) or (is_jz=0 and status=0)) and (weight<".$hd[0]['weight']." or wait_no=1) and withdraw_flag=0 and reg_id<>'".$hd[0]['reg_id']."'")->count();
			/**
			***患者再次报到处理流程
			**/
			if($hd[0]['baodao']==1){
				switch ($hd[0]['status']){
					...
					***/
					...
					if($hd[0]['is_jz']==1){//已经被接诊
						...
						$is_over = getDocIsOffLine($hd[0]['allot_doctor_code'],$hd[0]['dept_code'],$exp);
						switch($is_over){
							case 1://停诊状态
							$rel['is_over']=1;
							$rel['doctor_name'] = getDoctorNameByCode($hd[0]['allot_doctor_code']);						
							$rel['doctor_code'] = trim($hd[0]['fp_doctor_code']);
							$rel['reg_id'] = $hd[0]['reg_id'];
							$rel['dept_code'] = $hd[0]['dept_code'];
							break;
							case 0://开诊状态
							if($hd[0]['fp_doctor_code']!=""){//已经分配了医生的流程
								$data['status'] = 5;
								$rel['is_over']=0;
								if($hd[0]['reg_seq_no']>=1000){ 
									...
									$data['fp_doctor_name'] =  getDoctorNameByCode($fp_doc_ary['fp_doctor_code']);
									$data['room'] = $fp_doc_ary['room'];
									*/
									$fp_doc_ary['fp_doctor_code'] = $hd[0]['doctor_code'];
									$fp_doc_ar['room'] = M("client_uid")->where("uid=".$hd[0]['doctor_code'])->getField("room_id");
									$cda['op_flag'] = 1;
									$cda['op_message'] = "加号患者 ".$hd[0]['pat_name']."复诊成功";
									$rel['status_txt'] = "复诊成功<br>";  
								}else{
									//$fp_room = M("register_assign")->where("reg_id='".$hd[0]['reg_id']."'")->getField("room");
									$fp_room = $hd[0]['room'];
									if($w_num>1){
										$rel['status_txt'] = $my_dept_name.",复诊成功,前面等待".$w_num."人,请在侯诊区等候<br>";
									}else{
										if(trim($fp_room)==""){
											$rel['status_txt'] = $my_dept_name.",复诊成功,您的就诊医生当前未上线,请到分诊台咨询<br>";
										}else{
											$rel['status_txt'] = $my_dept_name.",复诊成功,前面等待".$w_num."人,请在".getShowRoomName($fp_room)."诊室门外等候<br>";	
										}
									}
									$cda['op_flag'] = 1;
									$cda['op_message'] = "复诊成功,前面等待".$w_num."人,请在第".getShowRoomName($fp_room)."诊室门外等候<br>";
								}
							}else{//没有分配医生的流程（即初诊未报到）
								if($hd[0]['doctor_name']!=""){//doctor_name有值 则直接分到该医生名下
									$fp_doc_ary['fp_doctor_code'] = getDoctorCodeByName($hd[0]['doctor_name']);
									$fp_doc_ary['room'] = M("client_uid")->where("uid=".getDoctorCodeByName($hd[0]['doctor_name']))->getField("room_id");
									$rel['status_txt'] = $my_dept_name.",您已报到,分配在第 <b>".$this->getRealRoomID($fp_doc_ary['room'])."</b> 诊室,前面等待".$w_num."人<br/>";
									$cda['op_message'] = $my_dept_name.",初诊未报到患者（有就诊医生）首次报到,分配在第 <b>".$this->getRealRoomID($fp_doc_ary['room'])."（".$fp_doc_ary['room']."）</b> 诊室";
									$cda['op_flag'] = 1;
								}else{//doctor_name无值则按规则分配就诊医生
									$fp_doc_ary = $this->getFpDoctor($hd,$hd[0]['doctor_code'],$zhenqu);
									if($fp_doc_ary['room']!=""&&$fp_doc_ary['room']!=0){
										$rel['status_txt'] = $my_dept_name.",再次报到,您被分配在第 <b>".$this->getRealRoomID($fp_doc_ary['room'])."</b> 诊室<br/>";
										$cda['op_message'] = $my_dept_name.",初诊未报到患者（无就诊医生）首次报到,您被分配在第 <b>".$this->getRealRoomID($fp_doc_ary['room'])."</b> 诊室";
										$cda['op_flag'] = 1;
									}else{	
										$rel['status_txt'] = $my_dept_name.",初次报到,前面等待".$w_num."人,请在候诊区等候<br>";
										$cda['op_flag'] = 1;
										$cda['op_message'] = $my_dept_name.",初诊未报到患者（无就诊医生）报到,前面等待".$w_num."人,请在候诊区等候";
									}
								}
								//$rel['error'] = $my_dept_name.",初诊未报到，请到分诊台处理";
								$rel['success']=1;
								$cda['op_flag'] = 1;
								$cda['op_message'] = "初诊未报到";
						    }
							break;
						}
					}else{
						$rel['status_txt'] = "您已经被叫,请去".$this->getRealRoomID($hd[0]['room'])."诊室就诊<br>";
					}
					break;
					case 3:
					if($hd[0]['is_jz']==1){
						...
						$rel['status_txt'] = "过号复诊成功<br>";
						$cda['op_flag'] = 1;
						$cda['op_message'] = "过号复诊成功";
					}else{
						$data['status'] = 0;//过号归队
						//开始分配医生
						$fp_doc_ary = $this->getFpDoctor($hd,$hd[0]['doctor_code'],$zhenqu);
						$data['fp_doctor_code'] = $fp_doc_ary['fp_doctor_code'];
						$data['room'] = $fp_doc_ary['room'];
						$rel['status_txt'] = $my_dept_name.",过号重排,您被分配在第<font color='red'>".$this->getRealRoomID($fp_doc_ary['room'])."</font>诊室,请及时就诊<br>";
						$cda['op_flag'] = 1;
						$cda['op_message'] = $my_dept_name.",过号重排,被分配在第".$this->getRealRoomID($fp_doc_ary['room'])."诊室<br>";
					}
					break;
					case 0://此种情况是患者报到后在就诊前又再次报到的情况
					if($hd[0]['is_jz']==1){
						if($hd[0]['fp_doctor_code']==""){
							...
							if($hd[0]['doctor_name']!=""){//doctor_name有值 则直接分到该医生名下
								...
								$fp_doc_ary['room'] = M("client_uid")->where("uid=".getDoctorCodeByName($hd[0]['doctor_name']))->getField("room_id");
								$rel['status_txt'] = $my_dept_name.",您已报到,分配在第 <b>".$this->getRealRoomID($fp_doc_ary['room'])."</b> 诊室,前面等待".$w_num."人<br/>";
								$cda['op_message'] = $my_dept_name.",未呼就接诊患者（有就诊医生）首次报到,分配在第 <b>".$this->getRealRoomID($fp_doc_ary['room'])."（".$fp_doc_ary['room']."）</b> 诊室";
								$cda['op_flag'] = 1;
							}else{//doctor_name无值则按规则分配就诊医生
								//开始分配医生
								$fp_doc_ary = $this->getFpDoctor($hd,$hd[0]['doctor_code'],$zhenqu);
								if($fp_doc_ary['room']!=""&&$fp_doc_ary['room']!=0){
									$rel['status_txt'] = $my_dept_name.",再次报到,请到 <b>".$this->getRealRoomID($fp_doc_ary['room'])."</b> 诊室门外候诊<br/>";
								}else{	
									$rel['status_txt'] = $my_dept_name.",再次报到,前面等待".$w_num."人,请在候诊区等候<br>";
								}
							}
							//$rel['status_txt'] = "初诊未报到,请到分诊台咨询<br>";
							$cda['op_flag'] = 1;
							$cda['op_message'] = "初诊未报到".$rel['status_txt'];
						}else{
							$data['status'] = 5;
							$fp_room = M("register_assign")->where("reg_id='".$hd[0]['reg_id']."'")->getField("room");
							$rel['status_txt'] = $my_dept_name.",复诊成功,请去第".getShowRoomName($fp_room)."诊室侯诊<br>";
							$cda['op_flag'] = 1;
							$cda['op_message'] = "复诊成功,请去第".getShowRoomName($fp_room)."诊室侯诊";
						}
					}else{
						//开始分配医生
						$fp_doc_ary = $this->getFpDoctor($hd,$hd[0]['doctor_code'],$zhenqu);
						if($fp_doc_ary['room']!=""&&$fp_doc_ary['room']!=0){
							...
							$rel['status_txt'] = $my_dept_name.",再次报到成功,您被分配在第 <b>".$this->getRealRoomID($fp_doc_ary['room'])."</b> 诊室<br />";
							$cda['op_flag'] = 1;
							$cda['op_message'] = $my_dept_name.",再次报到成功,您被分配在第".$this->getRealRoomID($fp_doc_ary['room'])."诊室";
						}else{
							//$rel['status_txt'] = "患者 ".$hd[0]['pat_name']."报到成功，正在等待分配诊室";
							$rel['status_txt'] = $my_dept_name.",再次报到成功,前面等待".$w_num."人,请在候诊区等候<br>";
							//$rel['status_txt'] = "报到成功,请在候诊区等候<br>";
							$cda['op_flag'] = 1;
							$cda['op_message'] = "再次报到成功,前面等待".$w_num."人,请在候诊区等候";
						}
			...
			****/
				...
				*****/
				if($hd[0]['is_jz']==1){
					$data['baodao']=1;
					$data['status']=5;
					if($hd[0]['reg_seq_no']<0){ //加号患者开始分配医生
						$fp_doc_ary = $this->getFpDoctor($hd,$hd[0]['doctor_code'],$zhenqu);
						$data['fp_doctor_code'] = $fp_doc_ary['fp_doctor_code'];
						$data['fp_doctor_name'] = getDoctorNameByCode($fp_doc_ary['fp_doctor_code']);
						...
						$rel['status_txt'] = "加号复诊成功";
						$data['status']=5;
						$cda['op_flag'] = 1;
						$cda['op_message'] = "加号复诊成功";
					}else{//初诊未报到且正常挂号患者（非加号）
						if($hd[0]['doctor_name']!=""){//doctor_name有值 则直接分到该医生名下
							$fp_doc_ary['fp_doctor_code'] = getDoctorCodeByName($hd[0]['doctor_name']);
							
							$fp_doc_ary['room'] = M("client_uid")->where("uid=".getDoctorCodeByName($hd[0]['doctor_name']))->getField("room_id");
							$rel['status_txt'] = $my_dept_name.",复诊报到,分配在第 <b>".$this->getRealRoomID($fp_doc_ary['room'])."</b> 诊室,前面等待".$w_num."人<br/>";
							$cda['op_message'] = $my_dept_name.",初诊未报到患者（有就诊医生）首次报到,分配在第 <b>".$this->getRealRoomID($fp_doc_ary['room'])."（".$fp_doc_ary['room']."）</b> 诊室";
							$cda['op_flag'] = 1;
							...
							$fp_doc_ary = $this->getFpDoctor($hd,$hd[0]['doctor_code'],$zhenqu);
							if($fp_doc_ary['room']!=""&&$fp_doc_ary['room']!=0){
								$rel['status_txt'] = $my_dept_name.",初次报到,您被分配在第 <b>".$this->getRealRoomID($fp_doc_ary['room'])."</b> 诊室<br/>";
								$cda['op_message'] = $my_dept_name.",初诊未报到患者（无就诊医生）首次报到,您被分配在第 <b>".$this->getRealRoomID($fp_doc_ary['room'])."</b> 诊室";
								$cda['op_flag'] = 1;
							}else{	
								$rel['status_txt'] = $my_dept_name.",初次报到,前面等待".$w_num."人,请在候诊区等候<br>";
								$cda['op_flag'] = 1;
								$cda['op_message'] = $my_dept_name.",初次报到,前面等待".$w_num."人,请在候诊区等候";
							}
						}
					...
					$fp_doc_ary = $this->getFpDoctor($hd,$hd[0]['doctor_code'],$zhenqu);
					if($fp_doc_ary['room']!=""&&$fp_doc_ary['room']!=0){
						$w_num = M("register_assign")->where("dept_code='".$hd[0]['dept_code']."' and reg_type='".$hd[0]['reg_type']."' and ((is_jz=1 and status=5) or (is_jz=0 and status=0)) and (weight<".$hd[0]['weight']." or wait_no=1) and reg_id<>'".$hd[0]['reg_id']."' and fp_doctor_code='".$fp_doc_ary['fp_doctor_code']."'")->count();
						//echo M("register_assign")->where("dept_code='".$hd[0]['dept_code']."' and reg_type='".$hd[0]['reg_type']."' and ((is_jz=1 and status=5) or (is_jz=0 and status=0)) and (weight<".$hd[0]['weight']." or wait_no=1) and reg_id<>'".$hd[0]['reg_id']."' and fp_doctor_code='".$fp_doc_ary['fp_doctor_code']."'")->select(false);
						$rel['status_txt'] = $my_dept_name.",初次报到,分配在第 <b>".$this->getRealRoomID($fp_doc_ary['room'])."</b> 诊室，前面有".$w_num."人在等待<br/>";
						$cda['op_flag'] = 1;
						$cda['op_message'] = $my_dept_name.",初次报到,被分配在第".$this->getRealRoomID($fp_doc_ary['room'])."诊室,前面有".$w_num."人在等待";
					}else{
						$rel['status_txt'] = $my_dept_name.",初次报到,您前面等待".$w_num."人,请在候诊区等候<br>";
						$cda['op_flag'] = 1;
						$cda['op_message'] = $my_dept_name.",初次报到成功,前面等待".$w_num."人,请在候诊区等候";
					}
				}
				...
				$data['room'] = $fp_doc_ary['room'];
				$data['wait_no'] = $fp_doc_ary['wait_no'];
				$data['bd_time'] = date("Y-m-d H:i:s");
			}
			
			M("register_assign")->where("reg_id='".$reg_id."'")->save($data);
			//echo  M("register_assign") ->getLastSql()."\r\n";
			unset($data);
...
public function getFpDoctor($hd,$doctor_code,$zhenqu){
	...
	if(trim($doctor_code)!=""){
		...
		switch($hd[0]['reg_type']){
			case "专家":
			$rel['room']= M("client_uid")->where("uid='".$doctor_code."' and dept='".$hd[0]['dept_code']."' and expert=1")->getField("room_id");
			break;
			case "普通":
			$rel['room']= M("client_uid")->where("uid='".trim($doctor_code)."' and dept='".$hd[0]['dept_code']."' and expert=0")->getField("room_id");
			break;
		}
	}else{
		$login_doctor_ary = M("client_uid")->where("dept='".$hd[0]['dept_code']."' and find_in_set(room_id,'".$zq_room."') and expert=0 and is_over=0")->select();
		if(!$login_doctor_ary){
			$rel['fp_doctor_code']="";
			$rel['room'] = "";
			return $rel;
		}	
		//判断比自己号小的是否还有未分配的患者，如果有就不分配只报到
		$no_fenpei_count = M("register_assign")->where("fp_doctor_code='' and reg_type='".$hd[0]['reg_type']."' and dept_code='".$hd[0]['dept_code']."' and withdraw_flag='0' and ((is_jz=1 and status=5) or (is_jz=0 and status=0)) and weight<".$hd[0]['weight'])->count();
		if($no_fenpei_count>0){
			$rel['fp_doctor_code']="";
			$rel['room'] = "";
			return $rel;
		}
		for($i=0;$i<count($login_doctor_ary);$i++){	
			//等候患者人数
			M()->query("update register_assign set fp_doctor_code='',fp_doctor_name='',room='' where reg_id='".$hd[0]['reg_id']."'");
			$fp_num = M("register_assign")->where("fp_doctor_code='".$login_doctor_ary[$i]['uid']."' and reg_type='普通' and dept_code='".$hd[0]['dept_code']."' and withdraw_flag=0 and baodao=1 and ((is_jz=1 and status=5) or (is_jz=0 and status=0))")->count();
			switch($fptype){
				case 1:
				...
				**/
				...
				if($fptype==1){
					for($n=0;$n<count($c_ary);$n++){	
						$p_wait_ary = M("register_assign")->where("fp_doctor_code='".$c_ary[$n]."' and reg_type='普通' and dept_code='".$hd[0]['dept_code']."' and withdraw_flag=0 and ((is_jz=1 and status=5) or (is_jz=0 and status=0))")->order("noon_flag,weight asc")->select();
						if(count($p_wait_ary)>0){ 
							for($z=0;$z<count($p_wait_ary);$z++){
								if($n==0){
									$doc[$z] = $c_ary[$n]."|".$p_wait_ary[$z]['weight'];
								}else{
									$doc[$z] .= ",".$c_ary[$n]."|".$p_wait_ary[$z]['weight'];  
								}
								$max[] = $p_wait_ary[$z]['weight'];
							}
						}/*else{
								$doc[] =$c_ary[$n]."|".$c_ary[$n]*100;
								$max[] = $c_ary[$n]*100;
						}*/
					}
					/*echo $hd[0]['reg_id'];
					...
					*/
					if($max){
						...
						foreach($doc as $val){
							...
							foreach($ary as $val2){
								...
								if($ary2[1]==$fp_no){
									...
									$fp_num_tmp =  M("register_assign")->where("fp_doctor_code='".$ary2[0]."' and reg_type='普通' and dept_code='".$hd[0]['dept_code']."' and withdraw_flag=0 and ((is_jz=1 and status=5) or (is_jz=0 and status=0))")->count();
								}
							}
						}
						//echo "分配的DOC:".$fp_num_tmp."|".$fp_no."\r\n"; 
						
						if($fp_no>=$hd[0]['weight']){
							$fp_doctor_code = $fp_doctor_code_tmp;
							$wait_no = 1;
						}else{
							if($get_fp_num>$fp_num_tmp){
								$fp_doctor_code = $fp_doctor_code_tmp;
								$wait_no = 1;
							}else{
								$fp_doctor_code="";
						...
						/*for($n=0;$n<count($c_ary);$n++){
							$p_num = M("register_assign")->where("fp_doctor_code='".$c_ary[$n]."' and reg_type='普通' and dept_code='".$hd[0]['dept_code']."' and withdraw_flag='0'")->count();
							if($p_num<$p_min_num){
								$p_min_num = $p_num;
						...
						$pat_wait_no_num = M("register_assign")->where("fp_doctor_code='' and dept_code='".$hd[0]['dept_code']."' and reg_type='".$hd[0]['reg_type']."' and weight<".$hd[0]['weight']." and ((is_jz=1 and status=5) or (is_jz=0 and status=0)) and withdraw_flag='0'")->count();
						if($pat_wait_no_num>0){
								$fp_doctor_code=''; 
						}else{
							for($n=0;$n<count($c_ary);$n++){
								$p_num = M("register_assign")->where("fp_doctor_code='".$c_ary[$n]."' and reg_type='普通' and dept_code='".$hd[0]['dept_code']."' and withdraw_flag='0'")->count();
	...
	**/
	if($fptype==1){
		$fp_num2 = M("register_assign")->where("fp_doctor_code='".$fp_doctor_code."' and withdraw_flag=0 and dept_code='".$hd[0]['dept_code']."' and baodao=1 and reg_type='普通' and reg_id!='".$hd[0]['reg_id']."' and  ((is_jz=1 and status=5) or (is_jz=0 and status=0))")->order("noon_flag,weight asc")->field("reg_id,reg_seq_no,weight,status")->select();
		//print_r($fp_num2);
