...
public function index(){
  ...
  for($i=0;$i<count($list);$i++){
	...
	if($list[$i]['expert']==1){
		$where="fp_doctor_code='".$doctor_code."' and reg_type='专家'";
	}else{
		$where="fp_doctor_code='".$doctor_code."' and reg_type='普通'";
	}
	...
	echo M("register_assign")->where("status=1 and ".$where." and withdraw_flag='0'")->limit(1)->order("call_time desc")->select(false);
	...
	if($relist[$i]['is_jz']==1){
		$relist[$i]['status_txt'] = '<font color="green"><b>已接诊</b></font>';
	}
	if($relist[$i]['status']==0){
		$relist[$i]['status_txt'] = '<font color="#333333"><b>未呼叫</b></font>';
	}
	if($relist[$i]['status']==3){
		$relist[$i]['status_txt'] = '<font color="blue"><b>已过号</b></font>';
	}
...
public function op_tingzhen(){
	...
	$expert = M("client_uid")->where($condition)->getField("expert");
	if($is_over==1){
		$is_over=0;
	}else{
		$is_over=1;
	...
	if($is_over==1){
		...
		if($expert==0){
			...
			M("register_assign")->where("fp_doctor_code='".$uid."' and baodao=1 and is_jz=0")->save($data2);
		}
		...
		if($expert==0){
			//首先确定自己已经分配了几名患者
			$my_fnum = M()->query("select count(*) as num from register_assign where fp_doctor_code='".$uid."' and ((status=5 and is_jz=1) or (status=0 and is_jz=0))");
			...
			foreach($lg1 as $val){
				M()->query("update register_assign set room='".$room."',fp_doctor_code='".$uid."',fp_doctor_name='".$doctor_name."' where reg_id='".$val['reg_id']."' and baodao=1 and dept_code='".$dept_code."'");
			}
		}
	}
