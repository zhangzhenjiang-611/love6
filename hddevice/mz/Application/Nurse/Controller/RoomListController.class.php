...
public function del(){
	...
	if(M("tbl_room_list")->where($condition)->delete()){
		...
		$this->success("删除诊室成功","/mz/Nurse/RoomList/");
	}else{
		$this->error("删除诊室失败");
	}
}
...
public function edit_do(){
	...
	$this->success("修改诊室信息成功","/mz/Nurse/RoomList");
...
public function add_do(){
	...
	if(0==0){
		...
		$this->success("添加诊室成功","/mz/index.php/Nurse/RoomList");
	}else{
		$this->error("此诊室已存在");
	}
...
public function imgs_upload(){
	...
	if(I("post.mode")=="edit"){
		...
		$datas['pic_url'] = "/Uploads/".$info['Filedata']['savepath'].$info['Filedata']['savename'];
