...
public function index(){
  ...
  $show = $Page->show();// 分页显示输出
	$list = M("doctor_info")->where("doctor_name like '%".I("post.keywords")."%'")->limit($Page->firstRow.','.$Page->listRows)->select();
  }else{
	  $count = M()->Table("doctor_info")->count();
	   $Page = new \Think\Page2($count,getOpVal("houtai_page_num"));
  $show = $Page->show();// 分页显示输出
  	$list = M("doctor_info")->limit($Page->firstRow.','.$Page->listRows)->select();
  }
