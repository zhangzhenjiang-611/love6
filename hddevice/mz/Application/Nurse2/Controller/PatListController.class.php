...
public function index(){
 ...
 if($flag=='search'){
  ...
  $count = M("register_assign")->where($where)->count();
  ...
  $list = M("register_assign")->where($where)->order("noon_flag desc,reg_seq_no desc")->limit($Page->firstRow.','.$Page->listRows)->select();
...
// M()->db(1,"DB_CONFIG1")->query("DROP TABLE #A");
  for($i=0;$i<count($list);$i++){
	 $pinfo = M("register_assign")->where("reg_id='".$list[$i]['reg_id']."'")->field("status,baodao,room")->select();
  ...
  for($i=0;$i<count($dept_list);$i++){
	 ...
	 $totals = M("register_assign")->where("dept_code='".$dept_list[$i]['dept_code']."'")->count();
	 $dept_list[$i]['total'] = $totals;
	  
	  $doclist = M("register_assign")->where("dept_code='".$dept_list[$i]['dept_code']."' and (doctor_code!='' or fp_doctor_code!='') and baodao=1")->group("doctor_code")->select();
	   for($m=0;$m<count($doclist);$m++){
		$doclist[$m]['counts'] = M("register_assign")->where("fp_doctor_code='".$doclist[$m]['fp_doctor_code']."'")->count(); 
	  }
