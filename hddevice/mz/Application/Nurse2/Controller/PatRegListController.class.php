...
class PatRegListController extends Controller {

public function index(){
	if(I("get.flag")=="search"){
  		$count = M()->Table("tbl_baodao_records")->where("pat_name like '%".I("post.keywords")."%'")->count();
		$Page = new \Think\Page2($count,getOpVal("houtai_page_num"));
 		$show = $Page->show();// 分页显示输出
		$list = M("tbl_baodao_records")->where("pat_name like '%".I("post.keywords")."%'")->limit($Page->firstRow.','.$Page->listRows)->select();
  	}else{
	   $count = M()->Table("tbl_baodao_records")->count();
	   $Page = new \Think\Page2($count,getOpVal("houtai_page_num"));
  	   $show = $Page->show();// 分页显示输出
  	   $list = M("tbl_baodao_records")->limit($Page->firstRow.','.$Page->listRows)->order("cdate desc")->select();
  }
