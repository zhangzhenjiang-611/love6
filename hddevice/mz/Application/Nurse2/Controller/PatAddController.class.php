...
public function insertPat($pat_code,$where2){
	$s_sql = "select * from view_patient_registration where pat_code='".$pat_code."' and ".$where2;
	...
	for($i=0;$i<count($row);$i++){
		...
		**/
		...
		if($row[$i]['reg_seq_no']<0){
			...
			$data['weight']	= $weight=1000+$weight;
		}else{
		...
		if(!$local_flag){
			if(trim($row[$i]['doctor_code'])!=""){
				...
				switch($row[$i]['reg_type']){
					...
					break;
					case "普通":
					$data['room']= M("client_uid")->where("uid='".trim($row[$i]['doctor_code'])."' and expert=0")->getField("room_id");
					break;
					default:
					$data['room'] = 0;
					break;
...
public function getPat(){
	...
	if(count($row)==0){
		...
		if(count($patlist)>0){
			$rel['success'] = 0;
			$rel['error'] = "患者 ".iconv("gbk","utf8",$patlist[0]['pat_name'])." 为非本诊区患者 请核对";
			/**
			**患者报到插入记录数据开始*************************************************
			**/
			$his_code = M("tbl_op")->where("op_name='his_code'")->getField("op_val");
	...
	if(count($row)==1){
		...
		$pat = M()->db(1,"DB_CONFIG1")->query("select * from view_patient_registration  where reg_id='".$row[0]['reg_id']."'");
		...
		$p_ary =  M()->db(1,"DB_CONFIG1")->query("select * from view_patient_registration  where pat_code='".$pat_code."' and ".$where2);
		for($i=0;$i<count($p_ary);$i++){
			...
			$p_ary[$i]['pat_sex'] = iconv("gbk","utf8",$p_ary[$i]['pat_sex']);
			...
			$p_ary[$i]['reg_time'] = date("Y-m-d H:i:s",strtotime($p_ary[$i]['reg_time']));
		}
		$rel['pat'] = $p_ary;
		$rel['num'] = count($p_ary);
		$rel['success'] = 1; 
			...
			if($hd[0]['baodao']==1){
				switch ($hd[0]['status']){
					...
					if($hd[0]['is_jz']==1){  
						...
						if($is_over==1){
							...
							if($hd[0]['fp_doctor_code']!=""){
								...
								if($hd[0]['reg_seq_no']>1000){ 
									...
									$rel['status_txt'] = "加号患者 ".$hd[0]['pat_name']."(".$hd[0]['dept_name'].")复诊成功";
								}else{
									$rel['status_txt'] = "患者 ".$hd[0]['pat_name']."(".$hd[0]['dept_name'].")复诊成功";
								}
							}else{
								$rel['error'] = "患者 ".$hd[0]['pat_name']."(".$hd[0]['dept_name'].")初诊未报到，请为其手动分配复诊医生";
								$rel['success']=0;
							}
						...
						$data['status'] = 1;
						$rel['status_txt'] = "患者 ".$hd[0]['pat_name']."(".$hd[0]['dept_name'].")已经叫号，请去<font color='red'>".$this->getRealRoomID($hd[0]['room'])."</font>诊室就诊";
					...
					if($hd[0]['is_jz']==1){
						$data['status'] = 5;//复诊
						$rel['status_txt'] = "过号患者 ".$hd[0]['pat_name']."(".$hd[0]['dept_name'].")复诊成功";
					}else{
						$data['status'] = 0;//过号归队
						//开始分配医生
						$fp_doc_ary = $this->getFpDoctor($hd,$hd[0]['doctor_code'],$zhenqu);
						$data['fp_doctor_code'] = $fp_doc_ary['fp_doctor_code'];
						$data['room'] = $fp_doc_ary['room'];
						$rel['status_txt'] = "过号患者 ".$hd[0]['pat_name']."(".$hd[0]['dept_name'].")重新分配在第<font color='red'>".$this->getRealRoomID($fp_doc_ary['room'])."</font>诊室";				
					}
					break;
					case 0:
					...
					if($hd[0]['is_jz']==1){
						if($hd[0]['fp_doctor_code']!=""){
							...
							$rel['error'] = "患者 ".$hd[0]['pat_name']."(".$hd[0]['dept_name'].")初诊未报到,请手动分配医生";
							$rel['success'] = 0;
						}
						...
						if($fp_doc_ary['room']!=""&&$fp_doc_ary['room']!=0){
							...
							$rel['status_txt'] = "患者 ".$hd[0]['pat_name']." (".$hd[0]['dept_name'].")报到成功,被分配在第 <b>".$this->getRealRoomID($fp_doc_ary['room'])."</b> 诊室";
						}else{
							$rel['status_txt'] = "<br>患者 ".$hd[0]['pat_name']."(".$hd[0]['dept_name'].")报到成功，正在等待分配诊室";
						}
				...
				if($hd[0]['is_jz']==1){
					...
					if($hd[0]['reg_seq_no']<0){ 
						...
						if($hd[0]['fp_doctor_code']==""){
							...
							$data['status']=5;
							$rel['status_txt'] = "<br>患者 ".$hd[0]['pat_name']."复诊成功!";
						}
					...
					$fp_doc_ary = $this->getFpDoctor($hd,$hd[0]['doctor_code'],$zhenqu);
					if($fp_doc_ary['room']!=""&&$fp_doc_ary['room']!=0){
						$rel['status_txt'] = "<br>患者 ".$hd[0]['pat_name']."(".$hd[0]['dept_name'].")报到成功，被分配在第 <b>".$this->getRealRoomID($fp_doc_ary['room'])."</b> 诊室";
					}else{
						$rel['status_txt'] = "<br>患者 ".$hd[0]['pat_name']."(".$hd[0]['dept_name'].")报到成功，等待分配诊室";
					}
...
public function getFpDoctor($hd,$doctor_code,$zhenqu){
	...
	if(trim($doctor_code)!=""){
		...
		for($i=0;$i<count($login_doctor_ary);$i++){	
			...
			$fp_num = M("register_assign")->where("fp_doctor_code='".$login_doctor_ary[$i]['uid']."' and reg_type='普通' and dept_code='".$hd[0]['dept_code']."' and withdraw_flag=0 and baodao=1 and ((is_jz=1 and status=5) or (is_jz=0 and status=0)) and reg_id!='".$hd[0]['reg_id']."'")->count();
			switch($fptype){
				case 1:
				...
				**/
				if($fptype==1){
					for($n=0;$n<count($c_ary);$n++){	
						$p_wait_ary = M("register_assign")->where("fp_doctor_code='".$c_ary[$n]."' and withdraw_flag=0 and dept_code='".$hd[0]['dept_code']."' and ((is_jz=1 and status=5) or (is_jz=0 and status=0))")->order("reg_seq_no asc")->select();
					...
					if($max){
						...
						for($n=0;$n<count($c_ary);$n++){
							$p_num = strtotime(M("register_assign")->where("fp_doctor_code='".$c_ary[$n]."' and reg_type='普通' and dept_code='".$hd[0]['dept_code']."' and withdraw_flag='0'")->order("call_time desc")->limit(1)->getField("call_time"));
							if($p_num<$p_min_num){
								$p_min_num = $p_num;
					...
					$min_ary = array();
					for($n=0;$n<count($c_ary);$n++){
						$p_num = strtotime(M("register_assign")->where("fp_doctor_code='".$c_ary[$n]."' and withdraw_flag='0' and dept_code='".$hd[0]['dept_code']."'")->order("call_time desc")->limit(1)->getField("call_time"));
						$min_ary[] = $p_num;
						if($p_num<$p_min_num){
							$p_min_num = $p_num;
							$fp_doctor_code = $c_ary[$n];
	...
	**/
	if($fptype==1){
		$fp_num2 = M("register_assign")->where("fp_doctor_code='".$fp_doctor_code."' and withdraw_flag=0 and baodao=1 and dept_code='".$hd[0]['dept_code']."' and reg_type='普通' and reg_id!='".$hd[0]['reg_id']."' and find_in_set(status,'0,5')")->order("reg_seq_no asc")->field("reg_id,reg_seq_no,status")->select();
...
public function change_doctor(){
	...
	$p_in = M("register_assign")->where("reg_id='".I("get.reg_id")."'")->count();
	if($p_in>0){
		$this->baodao=1;
		$this->zhenqu = $zhenqu;
		...
		$doclist = M("client_uid")->where("room_id in(".$room_list_txt.") and is_over=0 and dept='".$dept_code."'")->select();
			//echo M("client_uid")->where("room_id in(".$room_list_txt.") and is_over=0 and dept='".$dept_code."'")->select(false);
		$doclist_txt = "";
...
public function pat_chadui(){
	...
	if($pinf){
		...
		$patlist = M("register_assign")->where($where." and baodao=1 and ((is_jz=1 and status=5) or (is_jz=0 and status=0))")->field("reg_id,reg_seq_no,pat_code,pat_name,weight")->order("weight asc")->select();
...
public function ajax_change_doctor(){	
	...
	$s_sql = "select * from view_patient_registration where reg_id='".$reg_id."'";
	$p_info = M()->db(1,"DB_CONFIG1")->query($s_sql);
