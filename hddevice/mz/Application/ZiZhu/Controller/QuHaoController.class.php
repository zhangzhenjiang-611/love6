...
class QuHaoController extends CommonController {
...
public function abc(){
	$patient_id="000104652300";
	$unit_sn="0101010";
	$ffcTime=date("Y-m-d");
	$ampm="a";
	$soap_fu=new \SoapClient('http://211.159.153.49:80/yanglin/zfb/soap/Service.php?wsdl');
	//var_dump($soap_fu);
	$row=$soap_fu->yy_status($patient_id,$unit_sn,$ffcTime,$ampm);
	var_dump($row);exit;
	...
	$where['ampm'] =$ampm;
	$map["_complex"] = $where;
	$a=M("condition_logs")->db(2,"DB_CONFIG2")->where($map)->find();
	var_dump($a);exit;
}
	...
	}else
	{
		...
		}else
		{
			...
			if($cash!==0){
				switch($pay_type){
				case "alipay"://支付宝退费
				$a=M("alipay_logs")->where("trade_no='{$trade_no}'")->find();
				if($a){
					//var_dump($trade_no,$cash,$stream_no,$source);
					$soap = new \SoapClient('http://172.168.0.89/soap1/Service.php?wsdl');
					$row = $soap->refund($trade_no,$cash,$stream_no,$source);
