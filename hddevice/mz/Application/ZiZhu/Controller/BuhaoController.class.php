...
class BuhaoController extends CommonController {
...
public function getPatInfo(){
	...
	if($result_return['execute_flag']==0){
			$getCF = $this->getPatReservationRecord($datarow_ary['patient_id'],$datarow_ary['card_code'],$datarow_ary['card_no'],$zzj_id,$op_code);
			$rel['buhao'] = $getCF;
		}
	//var_dump($rel);exit;
...
//获取诊间加号记录
public function getPatReservationRecord($patient_id,$card_code,$card_no,$zzj_id,$op_code){
	//需要三个参数 病人id  卡类型 卡号 21代表就诊卡 20 代表社保卡
/*	$patient_id = I("post.patient_id");
	$card_code = I("post.card_code");
	$card_no = I("post.card_no");
...
<root><commitdata><data><datarow record_sn="124156" patient_id="000113964700" card_code="21" card_no="H186701158" req_type="01" gh_flag="3"/></data></commitdata><returndata><data><datarow patient_id="000113964700" charge_total="50" cash="50" zhzf="0" tczf="0" pay_seq="124156_-1"/></data></returndata><operateinfo><info method="YYT_GH_CALC" opt_id="ZZ001" opt_name="" opt_ip="172.168.61.11" opt_date="2018-03-09" guid="9110377373" token="AUTO-YYRMYY-2018-03-09"/></operateinfo><result><info execute_flag="0" execute_message="挂号划价成功!" account="2018-03-09 15:07:55"/></result></root>';
	*/
	$result = str_replace("gb2312","utf8",$result);
	/**************日记记录开始***********/
	$data['card_code'] = I("post.card_code");
...
function YbXmlParse(){
	...
	if($result_return['execute_flag']==0){
		...
		$patient_id=$datarow_ary['patient_id'];
		$code_no=$datarow_ary['code'];
		$card_code=$datarow_ary['card_code'];
		$zzj_id =I("post.zzj_id");
		$op_code =I("post.op_code");


		if($result_return['execute_flag']==0){
			$getCF = $this->getPatReservationRecord($patient_id,$card_code,$code_no,$zzj_id,$op_code);
			$rel['buhao'] = $getCF;
		}
	//var_dump($rel);exit;
	$this->ajaxReturn($rel,"JSON");
