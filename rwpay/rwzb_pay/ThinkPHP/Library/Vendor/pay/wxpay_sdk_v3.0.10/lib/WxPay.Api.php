...
class WxPayApi
{
	...
	{		
		...
		if($useCert == true){
			...
			//curl_setopt($ch,CURLOPT_SSLCERT, dirname(__FILE__).'/'.$sslCertPath);
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, $sslKeyPath);
			//curl_setopt($ch,CURLOPT_SSLKEY, dirname(__FILE__).'/'.$sslKeyPath);
		}
		//post提交方式
		...
		//记录日志
	    $fileName = dirname(__FILE__)."/../../../../../../Application/Runtime/Logs/curl_wxpay".date('Ymd') . '.log';
	    $log = '';
	    $log .= 'TIME:' . date('Y-m-d H:i:s') . "\n";
	    $log .= 'URL:' . $url . "\n";
	    $log .= "METHOD:POST\n";
	    $log .= 'HEADER:' . "\n";
	    $log .= 'PARAMS:' . (is_array($xml) ? json_encode($xml) : $xml) . "\n";
	    $log .= 'RETURN:' . $data . "\n";
	    $log .= 'ERROR_NO:' . $error_no . "\n";
	    $log .= 'ERROR:' . $error . "\n";
	    $log .= 'HTTP_CODE:' . $httpStatusCode . "\n";
	    $log .= "\t\n";
		...
		if($data){
			...
			curl_close($ch);
			throw new Exception("curl出错，错误码:$error_no,错误代码描述:$error");
		}
	}
