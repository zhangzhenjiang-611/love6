...
class MicroPay
{
	 ...
	 * @return 返回查询接口的结果
	 */
	public function pay($config, $microPayInput, $timeExpress)
	{
		//①、提交被扫支付
		$result = WxPayApi::micropay($config, $microPayInput, $timeExpress);
		//如果返回成功
		...
		{
			$succResult = 0;
			$queryResult = $this->query($out_trade_no, $succResult, $config);
			//如果需要等待1s后继续
			if($succResult == 2){
				sleep(1);
				continue;
		...
		//④、次确认失败，则撤销订单
		if(!$this->cancel($out_trade_no, $config))
		{
	 ...
	 */
	public function query($out_trade_no, &$succCode, $config)
	{
	 ...
	 * @param 调用深度 $depth
	 */
	public function cancel($out_trade_no, $config, $depth = 0)
	{
		try {
			if($depth > 10){
				return false;
