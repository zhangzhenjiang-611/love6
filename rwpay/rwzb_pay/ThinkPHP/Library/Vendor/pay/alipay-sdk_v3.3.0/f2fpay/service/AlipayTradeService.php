...
require_once dirname ( __FILE__ ).DIRECTORY_SEPARATOR.'../model/result/AlipayF2FCancelResult.php';
...
class AlipayTradeService {
	...
	public function queryTradeResult($req){
		...
		if($response->trade_status == "WAIT_BUYER_PAY" || $response->sub_code == "ACQ.TRADE_NOT_EXIST")
		{
			$result->setTradeStatus("WAIT_BUYER_PAY");
		}
		else if($this->querySuccess($response)){
			// 查询返回该订单交易支付成功
			$result->setTradeStatus("SUCCESS");
	...
	// 当面付2.0消费查询
	public function cancelTradeResult($req){
		$response = $this->cancel($req);
		$result = new AlipayF2FCancelResult($response);
	...
	function writeLog($text) {
		...
		//$text = characet ( $text );
		//file_put_contents ( dirname ( __FILE__ ).DIRECTORY_SEPARATOR.'../log/alipay'.date('Y-m-d'), date ( "Y-m-d H:i:s" ) . "  " . $text . "\r\n", FILE_APPEND );
	}
