...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		//Log::write("传入微信调用参数：".$inputObj->IsTrade_typeSet());
		//检测必填参数
		...
		if($inputObj->GetTrade_type() == "NATIVE" && !$inputObj->IsProduct_idSet()){
			//Log::write("统一支付接口中，缺少必填参数product_id！trade_type为JSAPI时，product_id为必填参数");
		...
		$inputObj->SetSubMch_id(WxPayConfig::SUB_MCHID);//商户号
	...
	public static function downloadBill($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsBill_dateSet()) {
			\Log::write("对账单接口中，缺少必填参数bill_date！");
		...
		\Log::write(var_export($response ,true)); 
		return $response;
	}
	...
	{		
		...
		if($data){
			...
			$error = curl_errno($ch);
			curl_close($ch);
			Log::write('curl_error:'.$ch.'curl_error'.curl_error($ch));
