...
class YuYueBaoDaoController extends CommonController {
public function his_record(){
    $row = M()->db(1,"DB_CONFIG1")->query("select * from v_yyt_jydz_rw_check where trade_no='4200000279201901297446080759'");
    // var_dump($row);exit;
...
public function yuyue_xinxi($card_no){
	$s_sql = "SELECT * FROM V_ZZJ_YYGH_BDCX where txm = '".$card_no."' order by sjd asc";
	/**************日记记录开始**************/
	$data['card_code'] = I("post.card_code");
	$data['patient_id'] = I("post.patient_id");
	$data['card_no'] = I("post.card_no");
