...
class JianKaController extends CommonController {
...
/***********************判断二代身份证是否已办卡****************************/
public function jianka_sfz_chaxun(){
	...
	$patient_sfz = I("post.patient_sfz");
	// $patient_sfz = '41152319940919'.rand(1000,9999);
	$patient_birthday = I("post.patient_birthday");
	$d=strtotime($patient_birthday);
	$patient_birthday = date("Y-m-d",$d);
	$patient_address = I("post.patient_address");
	$card_no = $patient_sfz;
	...
	$str = '<Request>
			...
			<Body>
				...
				<IDCardNo>'.$patient_sfz.'</IDCardNo>
				<Name>'.$patient_name.'</Name>
				<Sex>'.$patient_sex.'</Sex>
				<Birthday>'.$patient_birthday.'</Birthday>
				<Age></Age>
			</Body>
			</Request>';
	/**************日记记录开始**************/
...
/***********************身份证办卡****************************/
public function jianka_sfz_save(){
	$op_code = I("post.op_code");
	$zzj_id = I("post.zzj_id");
	$patient_phone = I("post.patient_phone");
	$patient_type = I("post.patient_type");
	...
	// $patient_sfz = '41152319940919'.rand(1000,9999);
	$patient_sfz = I("post.patient_sfz");
	// $patient_sfz = '110123199011288460';
	$patient_birthday = I("post.patient_birthday");
	$d=strtotime($patient_birthday);
	$patient_birthday = date("Y-m-d",$d);
...
public function yibao_save1(){
	$rel['result']['info']['resultcode']=0;
	$rel['result']['info']['errormsg']="失败";
...
public function yibao_save(){
	$op_code = I("post.op_code");
	$zzj_id = I("post.zzj_id");
	$patient_phone = I("post.yibao_phone");
	$patient_type = "4";
	$patient_name = I("post.yibao_name");
	$patient_sex = I("post.yibao_sex");
	$patient_nian = I("post.yibao_nian");
	$patient_yue = I("post.yibao_yue");
	$patient_ri = I("post.yibao_ri");
	$patient_birthday = $patient_nian."-".$patient_yue."-".$patient_ri;
	if($patient_sex=="男"){
		$patient_sex = "1";
	}else{
		$patient_sex = "2";
	}
	$patient_sfz = I("post.yibao_sfz");
	$patient_cardno = I("post.yibao_cardno");
	$patient_address = I("post.yibao_address");
	$ybsch = I("post.ybsch");//医保手册号
				...
				<commitdata>
					<data>
						<datarow>
							...
							<card_no>'.$patient_cardno.'</card_no>
							<social_no>'.$patient_sfz.'</social_no>
							<name>'.$patient_name.'</name> 
							<sex>'.$patient_sex.'</sex>
							<Birthday>'.$patient_birthday.'</Birthday>  
							<address>'.$patient_address.'</address>
							<shenfen_type>'.$patient_type.'</shenfen_type>
