...
class DubleDrugController extends Controller {
...
// 数据库连接结束
  ...
  $pagesize='5';
  // 查询总记录条数
 $return = odbc_exec($conn,"call web.DHCViewSelfServiceInterface_FindDrugItem('18','".$name."')");
 $num=0;
...
// print_r($yaowu);
 ...
 $name=I("post.str");
  ...
  $nums=M('drug_zcheng')->where($where)->count();
  //取得总页数
  ...
  // 当前页码
  $page='0';
  $yaowu = $model->query("SELECT * FROM `drug_zcheng` WHERE `yaowu_py` = ".$name." limit 0,5");
    $plist['page']='0';
  }else{
      $pages=ceil($nums/$pagesize);
  // 当前页码
  $page='1';
  $yaowu = $model->query("SELECT * FROM `drug_zcheng` WHERE `yaowu_py` LIKE '%".$name."%' limit 0,5");
  $plist['page']='1';
}
// print_r($yaowu);
  $plist['yaowu']=$yaowu;
...
$pagesize='5';
  ...
  $kaishi=($page-1)*$pagesize;
  $yaowu=M('drug_zcheng')->where($where)->limit($kaishi,$pagesize)->select();
    // echo M('drug')->_sql();
  $plist['yaowu']=$yaowu;
  $plist['success']='1';
  $plist['page']=$page;
